...
from app.services.ai_service import ai_service, create_coupon, validate_and_apply_coupon
...
def predict_product_price(product: dict):
    return ai_service.predict_price(product)
...
def validate_coupon(payload: schemas.CouponValidate, db: Session = Depends(get_db)):
    coupon = validate_and_apply_coupon(db, payload.code, payload.order_amount)
    return {
        "valid": True,
        "code": coupon.code,
